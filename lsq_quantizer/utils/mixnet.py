...
def Conv3x3Bn(in_channels, out_channels, stride, non_linear='ReLU', constr_activation=None, quantize_w=True, bw_act=None):
    if quantize_w:
        if non_linear=="ReLU":
            return nn.Sequential(
                Conv2d(in_channels, out_channels, 3, stride, 1, bias=False),
                nn.BatchNorm2d(out_channels),
                LsqActivation(constr_activation) if constr_activation is not None else nn.ReLU(inplace=True)
            )
        else:
            return nn.Sequential(
                Conv2d(in_channels, out_channels, 3, stride, 1, bias=False),
                nn.BatchNorm2d(out_channels),
                LsqActivation(get_constraint(bw_act, "swish")) if constr_activation is not None else nn.ReLU(inplace=True)
            )
    else:
        if non_linear=="ReLU":
            return nn.Sequential(
...
def Conv1x1Bn(in_channels, out_channels, non_linear='ReLU', constr_activation=None, bw_act=None):
    if non_linear=="ReLU":
        return nn.Sequential(
            Conv2d(in_channels, out_channels, 1, 1, 0, bias=False),
            nn.BatchNorm2d(out_channels),
            LsqActivation(constr_activation) if constr_activation is not None else nn.ReLU(inplace=True)
        )
    else:
...
class SqueezeAndExcite(nn.Module):
    def __init__(self, channels, squeeze_channels, se_ratio, constr_activation=None, bw_act=None):
        ...
        self.pool_act_quant = LsqActivation(constr_activation) if constr_activation is not None else nn.ReLU(inplace=True)
        self.se_reduce = Conv2d(channels, squeeze_channels, 1, 1, 0, bias=True)
        self.non_linear1 = LsqActivation(get_constraint(bw_act, "swish")) if constr_activation is not None else nn.ReLU(inplace=True)
        self.se_expand = Conv2d(squeeze_channels, channels, 1, 1, 0, bias=True)
        self.non_linear2 = nn.Sigmoid()
        self.post_squeeze_quant = LsqActivation(constr_activation) if constr_activation is not None else nn.ReLU(inplace=True)

    def forward(self, x):
        y = torch.mean(x, (2, 3), keepdim=True)
        y = self.pool_act_quant(y)
        y = self.non_linear1(self.se_reduce(y))
        y = self.non_linear2(self.se_expand(y))
        y = x * y
        y = self.post_squeeze_quant(y)
...
class GroupedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        for i in range(self.num_groups):
            self.grouped_conv.append(Conv2d(
...
class MixNetBlock(nn.Module):
    def __init__(
        ...
        ### INPUT ACTIVATIONS TO THIS BLOCK SHOULD ALREADY BE QUANTIZED
        ...
        if se:
            # squeeze and excite
            squeeze_excite = SqueezeAndExcite(expand_channels, in_channels, se_ratio, constr_activation=constr_activation, bw_act=bw_act)
        ...
        self.out_act_quant = LsqActivation(get_constraint(bw_act, "weight")) if constr_activation is not None else _Identity()

    def forward(self, x):
        if self.residual_connection:
            return self.out_act_quant(x + self.conv(x))
        else:
            return self.out_act_quant(self.conv(x))
...
def mixnet_s(quan_first=False,
             quan_last=False,
             constr_activation=None,
             preactivation=None,
             bw_act=None):
    return MixNet(net_type="mixnet_s", quan_first=quan_first, quan_last=quan_last, constr_activation=constr_activation, bw_act=bw_act)
